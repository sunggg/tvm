...
def visualize_network(expr, file_name):
    ...
    for node, node_idx in node_dict.items():
        ...
        if isinstance(node, relay.Function):
            ...
            dot.node(
                str(node_idx),
                f"{node.name_hint} {node_idx_str}:\n{tensor_info}",
                shape="rectangle",
            )
        elif isinstance(node, relay.expr.GlobalVar):
            dot.node(str(node_idx), f"{node.name_hint} {node_idx_str}", shape="rectangle")

        elif isinstance(node, relay.Constant):
            dot.node(
                str(node_idx),
                f"Constant {node_idx_str}:\nTensor[{tuple(node.data.shape)}, {node.data.dtype}]",
                shape="rectangle",
            )
            ...
            if isinstance(node.op, tvm.relay.Function):
                dot.node(
                    str(node_idx),
                    f"Call {node_idx_str}(Function({node_dict[node.op.body]}))",
                    shape="ellipse",
                    style="filled",
                    color=node_color,
                )
                ...
                if isinstance(node.op, relay.expr.GlobalVar):
                    dot.node(
                        str(node_idx),
                        f"Call{node_idx_str}(GlobalVar={node.op.name_hint})",
                        shape="ellipse",
                        style="filled",
                        color=node_color,
                    )
                    ...
                    dot.node(
                        str(node_idx),
                        f"Call {node_idx_str}(Var={node.op.name_hint})",
                        shape="ellipse",
                        style="filled",
                        color=node_color,
                    )
                    ...
                    dot.node(
                        str(node_idx),
                        f"Call {node_idx_str}(op={node.op.name})",
                        shape="ellipse",
                        style="filled",
                        color=node_color,
                    )
            ...
            dot.node(
                str(node_idx),
                f"TupleGetItem {node_idx_str}(idx={node.index})",
                shape="ellipse",
                style="filled",
                color=node_color,
            )
            ...
            dot.node(
                str(node_idx),
                f"Tuple {node_idx_str}(fileds=none)",
                shape="ellipse",
                style="filled",
                color=node_color,
            )
